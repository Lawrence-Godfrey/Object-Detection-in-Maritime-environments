		...
		if frame_counter > vid_fps//frame_rate:
			...
			if available:
				...
				if convert_to_flow:
					...
					frame = cv2.normalize(mag,None,0,255,cv2.NORM_MINMAX).astype(np.uint8)
...
for i in range(half_window, x_train.shape[0] - half_window - 1, half_window + 1):
	temp = np.zeros((frame_size, frame_size, frame_window*2), dtype=np.uint8)
...
# 		cv2.imshow('raw', x_train[i,:,:,j*2])
# 		cv2.imshow('flow', (x_train[i,:,:,j*2+1]/np.max(x_train[i,:,:,j*2+1]))*255)
