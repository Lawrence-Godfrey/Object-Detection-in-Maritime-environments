...
def Unet3D_small(input_shape, n_filters = 16, dropout = 0.1, batchnorm=True):
	input_layer = Input(input_shape, name='input')
	...
	c5 = conv3d_block(p3, n_filters = n_filters * 8, batchnorm=batchnorm)
...
def Unet3D_multi_out(input_shape, n_filters = 16, dropout = 0.1, batchnorm=True):
	input_layer = Input(input_shape, name='input')
	...
	outputs = Conv3D(1, (1, 1, 1), activation='sigmoid')(c10)
	print(outputs.shape)
	model = Model(inputs=[input_layer], outputs=[outputs[..., 0, 0], outputs[..., 1, 0], outputs[..., 2, 0], outputs[..., 3, 0], outputs[..., 4, 0], outputs[..., 5, 0], outputs[..., 6, 0], outputs[..., 7, 0]])
	return model