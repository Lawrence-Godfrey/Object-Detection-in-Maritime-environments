...
def Unet3D_short(input_shape, n_filters = 16, dropout = 0.1, batchnorm=True):
	input_layer = Input(input_shape, name='input')
	...
	c1 = Conv3D(filters = n_filters * 2, kernel_size = (3, 3, 3), padding='same')(p0)
	if batchnorm:
		c1 = BatchNormalization()(c1)
	c1 = Activation('relu')(c1)
	...
	c2 = conv3d_block(p1, n_filters * 4, batchnorm=batchnorm)
	p2 = MaxPool3D((2, 2, 2))(c2)
	p2 = SpatialDropout3D(dropout)(p2)



	c5 = conv3d_block(p2, n_filters = n_filters * 8, batchnorm=batchnorm)

	# Expansive Path


	u8 = Conv3DTranspose(n_filters * 4, (3, 3, 3), strides=(2, 2, 2), padding = 'SAME')(c5)
	u8 = concatenate([u8, c2])
	u8 = SpatialDropout3D(dropout)(u8)
	c8 = conv3d_block(u8, n_filters * 4, batchnorm=batchnorm)

	u9 = Conv3DTranspose(n_filters * 2, (3, 3, 3), strides=(2, 2, 2), padding = 'SAME')(c8)
	u9 = concatenate([u9, c1])
	u9 = SpatialDropout3D(dropout)(u9)
	c9 = conv3d_block(u9, n_filters * 2, batchnorm=batchnorm)
...
def Unet3D_single_out(input_shape, n_filters = 16, dropout = 0.1, batchnorm=True):
	input_layer = Input(input_shape, name='input')
	...
	c10 = Conv3D(filters = n_filters * 1, kernel_size = (3, 3, 3), padding='same')(c10)
	if batchnorm:
		c10 = BatchNormalization()(c10)
	c10 = Activation('relu')(c10)
	c10 = MaxPool3D((1, 1, 2))(c10)
	c10 = SpatialDropout3D(dropout)(c10)
